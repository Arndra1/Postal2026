    ...
    <tr className="border-t border-border hover:bg-muted/30">
      <td className="px-5 py-3">
        ...
        {(r.person_name || r.city || r.state) && (
          <div className="text-xs text-muted-foreground">{[r.person_name, r.city, r.state].filter(Boolean).join(" · ")}</div>
        )}
      ...
      <td className="px-5 py-3">
        <div className="flex items-center justify-end gap-1">
          ...
          <Button variant="ghost" size="sm" onClick={() => onEnrich(r)} disabled={enriching} title="Enrich (5 credits on success)">
