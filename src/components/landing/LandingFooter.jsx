...
export default function LandingFooter() {
  return (
    <footer className="border-t border-white/30 glass-nav">
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div>
            <p className="text-sm font-semibold mb-3 text-foreground">Explore</p>
            <ul className="space-y-2 text-sm text-muted-foreground">
              {exploreLinks.map((l) => (
                <li key={l.label}>
                  {l.href.startsWith("/") ? (
                    <Link to={l.href} className="hover:text-primary transition">{l.label}</Link>
                  ) : (
          ...
          <div>
            <p className="text-sm font-semibold mb-3 text-foreground">Company</p>
            <ul className="space-y-2 text-sm text-muted-foreground">
              {legalLinks.map((l) => (
                <li key={l.label}>
                  {l.href.startsWith("/") ? (
